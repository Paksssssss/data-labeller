...
// Segment Queries
var segmentQuery1 = 'SELECT news_id, news_title,news_date, news_content, news_image, news_url FROM news WHERE s_status=? ORDER BY news_date DESC LIMIT 4';
...
var segmentCountQuery = 'SELECT COUNT(*) FROM news WHERE s_status=2'
...
var newsModel = {
    ...
    startLabel: function(news_id, type) {
        if (type === "segment") {
            ...
            return new Promise(function(resolveUpdate, rejectUpdate) {
                mysql.query(platformUpdateQuery, [1, news_id], function(error, results, fields) {
                    segmentUpdateQuery
                    if (error) {
                        rejectUpdate(error.toString());
                        console.log(error.toString());
    ...
    getCount: function() {
        return new Promise(function(resolveQuery,rejectQuery){
            mysql.query(segmentCountQuery, function(error, results, fields){
                if (error){
                    console.log(error.toString());
                    rejectQuery(error.toString());
                }
